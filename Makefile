...
CXXFLAGS += -Iinclude -Ithird_party   # ETL / CLI11 / doctest / nlohmann-json headers
...
CLI_BIN  := build/viatext-cli
CLI_DEPS := # (no dep-gen for single-file CLI; optional to add)
...
# CLI links against the static core lib
$(CLI_BIN): $(LIB) $(CLI_SRCS)
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS) $(CLI_SRCS) $(LIB) -o $@ $(LDFLAGS)

# Convenience target to build only the CLI (and the lib it needs)
cli: $(CLI_BIN)
...
help:
	@echo 'Targets:'
	@echo '  make / make all  Build static lib and CLI binary'
	@echo '  make cli         Build only the CLI (and lib if needed)'
	@echo '  make test        Build and run tests (requires doctest)'
	@echo '                   Example: make test RUN="-tc=\"Ping*\" -s"'
	@echo '  make clean       Remove build artifacts'
	@echo ''
	@echo 'Options:'
	@echo '  SAN=1            Enable ASAN/UBSAN (both lib and CLI)'

# Auto-include header dependencies (generated by -MMD -MP for lib sources)
-include $(LIB_DEPS)
